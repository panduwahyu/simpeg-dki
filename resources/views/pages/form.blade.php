<x-layout bodyClass="g-sidenav-show bg-gray-200">
                    ...
                    <div class="card my-4">
                        ...
                        <div class="card-body px-4 pb-2">
                            ...
                            <form id="dokumenForm" action="{{ route('form.store') }}" method="POST">
                                ...
                                <!-- Checkbox Pegawai (muncul jika Pegawai Tertentu) -->
                                <div class="mb-3" id="pegawai_checkbox_container" style="display:none;">
                            ...
                            <table class="table table-bordered table-striped">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Tipe Periode</th>
                                        <th>Tahun</th>
                                        <th>Periode Terkait</th>
                                        <th>Aksi</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach($jenisDokumen as $index => $jd)
                                        @php
                                            $periodeList = \DB::table('mandatory_uploads')
                                                ->join('periode', 'mandatory_uploads.periode_id', '=', 'periode.id')
                                                ->where('mandatory_uploads.jenis_dokumen_id', $jd->id)
                                                ->select('periode.*')
                                                ->get();
                                        @endphp
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @if($periode->tipe == 'bulanan')
                                                        {{ \Carbon\Carbon::create($periode->tahun, $periode->bulan, 1)->locale('id')->translatedFormat('F Y') }}
                                                    @elseif($periode->tipe == 'triwulanan')
                                                        Triwulan {{ $periode->triwulan }} {{ $periode->tahun }}
                                                    @else
                                                        Tahun {{ $periode->tahun }}
                                                    @endif
                                                </td>
    ...
    <script>
        ...
        window.addEventListener('DOMContentLoaded', () => {
            togglePegawaiCheckbox();
        });

        // Validasi JS sebelum submit pakai SweetAlert2
            ...
            if (pegawaiType.value === 'specific') {
                ...
                if (!isChecked) {
                    ...
                    Swal.fire({
                        icon: 'warning',
                        title: 'Peringatan',
                        text: 'Minimal pilih 1 pegawai jika memilih "Pegawai Tertentu".',
